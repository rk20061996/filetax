...
exports.rejectUploadedDoc = (req,res) =>{
    document.getUserDataByToken({ user_id: req.user_id }, (err, data) => {
        console.log("user---?1", req.user_id)
        if (err) {
            res.status(200).send({
            ...
            console.log("dataAdmin Reject",)
            if(data[0].user_type == 1){
                admin.rejectUploadedDoc({ id: req.body.id,comment:req.body.comment }, (err, data) => {
                    // console.log("user---?1", user)
                    if (err) {
                        res.status(200).send({
                            status: 500,
...
exports.getAllUploadedDocument = (req,res) =>{
    document.getUserDataByToken({ user_id: req.user_id }, (err, data) => {
        console.log("user---?1", req.user_id)
        if (err) {
            res.status(200).send({
            ...
            console.log("dataAdmin",)
            if(data[0].user_type == 1){
                admin.getAllUploadedDocument({  }, (err, data) => {
                    // console.log("user---?1", user)
                    if (err) {
                        res.status(200).send({
                            status: 500,
...
exports.getAllNotification= (req,res) =>{
    document.getUserDataByToken({ user_id: req.user_id }, (err, data) => {
        ...
        if (err) {
            ...
            console.log("dataAdmin",)
            if(data[0].user_type == 1){
                admin.getAllNotification({  }, (err, data) => {
                    // console.log("user---?1", user)
                    if (err) {
                        res.status(200).send({
                            status: 500,
